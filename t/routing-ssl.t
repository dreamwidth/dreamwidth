...
handle_custom(
    "/ssl_test",
    name   => "ssl possible, not ssl, POST",
    method => "POST",
    final  => sub {
        my ( $r, $rv ) = @_;
        plan( tests => 1 );
...
handle_custom(
    "/ssl_test",
    name  => "ssl possible, not ssl, user page",
    opts  => { username => 'example' },
    final => sub {
        my ( $r, $rv ) = @_;
        plan( tests => 1 );
        is( $rv, $r->REDIRECT );
