...
is( $run->("email_alias show $user"), "error: You are not authorized to run this command." );
$u->grant_priv("reset_email");

is( $run->("email_alias show $user"), "error: $alias is not currently defined." );

is(
    $run->("email_alias set $user testing\@example.com"),
    "success: Successfully set $alias => testing\@example.com"
);

is( $run->("email_alias show $user"), "success: $alias aliases to testing\@example.com" );

is( $run->("email_alias delete $user"), "success: Successfully deleted $alias alias." );
