...
$LJ::CAP{$_}->{moodthemecreate} = 1 foreach ( 0 .. 15 );
...
SKIP: {
    ...
    my $themeid =
        $u->create_moodtheme( "testing", "testing a custom mood theme with missing moods." );
    ...
    my $err;
    $customtheme->set_picture( $testmoodid,
        { picurl => "http://example.com/moodpic", width => 10, height => 20 }, \$err );

    $vars = DW::Controller::Entry::_init( { remote => $u } );
    is( $vars->{moodtheme}->{id}, $customtheme->id, "Custom mood theme." );
    is( scalar keys %{ $vars->{moodtheme}->{pics} },
        1, "Only provide picture information for moods with valid pictures." );
    is( $vars->{moodtheme}->{pics}->{$testmoodid}->{pic},
        "http://example.com/moodpic", "Confirm picture URL matches." );
    is( $vars->{moodtheme}->{pics}->{$testmoodid}->{width}, 10, "Confirm picture width matches." );
    is( $vars->{moodtheme}->{pics}->{$testmoodid}->{height}, 20,
        "Confirm picture height matches." );
    is(
        $vars->{moodtheme}->{pics}->{$testmoodid}->{name},
        $moods->{$testmoodid}->{name},
        "Confirm mood name matches."
    );
    ...
    ok( $vars->{journallist}->[0]->equals($u), "First journal in the list is yourself." );
    ok(
        $vars->{journallist}->[1]->equals($comm_canpost),
        "Second journal in the list is a community you can post to."
    );
...
note(" Usejournal - init");
{
    ...
    ok(
        $vars->{journallist}->[0]->equals($comm_canpost),
        "Only item in the list is usejournal value."
    );
    ...
    is( $vars->{security}->[0]->{label}, '.select.security.public.label', "Public security" );
    is( $vars->{security}->[0]->{value}, "public",                        "Public security" );
    is( $vars->{security}->[1]->{label}, '.select.security.members.label',
        "Members-only security" );
    is( $vars->{security}->[1]->{value}, "access",                       "Access-only security" );
    ...
    # TODO:
    # tags ( fetched by JS )
    # mood, comments, age restriction don't change
    ...
    is( scalar @{ $vars->{journallist} }, 1, "Usejournal." );
    ok(
        $vars->{journallist}->[0]->equals($comm_nopost),
        "Only item in the list is usejournal value."
    );
...
note("Post - subject");
{
    ...
    is_deeply( $req, { %$postdecoded_bare, subject => $postdata->{subject}, } );
    is_deeply( $errors->get_all, [], "no errors" );
...
note("Post - security:public");
{
    ...
    is_deeply( $req, { %$postdecoded_bare, security => "public", }, "decoded entry form" );
...
note("Post - security:private");
{
    ...
    is_deeply(
        $req,
        { %$postdecoded_bare, security => "private" },
...
{
    my ( $req, $res, $u ) = post_with( taglist => "foo, bar, baz", );
    is_deeply(
        $req,
        {
            %$postdecoded_bare,
