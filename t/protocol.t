...
use Test::More skip_all =>
    "Test is not deterministic -- inconsistent results from content filters";    #tests => 243;
...
note("getcircle");
{
    ...
    is( scalar keys %$res,
        0, "Empty circle; no arguments provided to select the subset of the circle." );
    ...
    while ( my ( $include, $val ) = each %circle_args ) {
        ...
        is(
            scalar @response_users,
            scalar @cached_users,
            "Matched the number of users who are watching/trusting."
        );
        ...
        foreach my $user (@response_users) {
            ok(
                ++$cached_users{ $user->{username} },
                "User from response is expected to be there."
            );
        }
    ...
    while ( my ( $include, $val ) = each %circle_args ) {
        my @cached_users = @{ $val->[1] };
        my $limit =
            scalar @cached_users > $LJ::MAX_WT_EDGES_LOAD
        ...
        my @response_users = @{ $res->{ $val->[0] } };
        is( scalar @response_users,
            $limit, "Check that the number of users who can be fetched is limited." );
        my %cached_users = map { $_->user => 0 } @cached_users;
        foreach my $user (@response_users) {
            ok(
                ++$cached_users{ $user->{username} },
    ...
    is( scalar keys %$res, 1, "One key: " . ( keys %$res )[0] );
    ok( ref $res->{contentfilters} eq "ARRAY" && scalar @{ $res->{contentfilters} } == 0,
        "Empty list of content filters for this user." );
...
note("editcircle");
{
    ...
    ( $res, $err ) =
        $do_request->( "editcircle", username => $u->user, settrustgroups => \%trustgroups );
    $success->("Set trust groups.");
    is( scalar keys %$res, 0, "No response expected." );

    ( $res, $err ) = $do_request->( "getcircle", username => $u->user, includetrustgroups => 1 );
    is( scalar @{ $res->{trustgroups} }, scalar keys %trustgroups,
        "Number of trust groups match." );
    foreach my $trustgroup ( @{ $res->{trustgroups} } ) {
        ...
        is(
            $trustgroup->{public},
            $orig_trustgroup->{public} || 0,
            "Trustgroup public setting matches."
        );
        is(
            $trustgroup->{sortorder},
            $orig_trustgroup->{sort} || 50,
            "Trustgroup sortorder matches."
        );
    }
    ...
    ( $res, $err ) = $do_request->(
        "editcircle",
        username       => $u->user,
        settrustgroups => { 10 => $edited, 20 => $new }
    );
    ...
    ( $res, $err ) =
        $do_request->( "editcircle", username => $u->user, deletetrustgroups => [ 10, 5 ] );
    ...
    ( $res, $err ) = $do_request->(
        "editcircle",
        username => $u->user,
        add      => [ { username => $watchertruster->user } ]
    );
    ( $res, $err ) = $do_request->(
        "getcircle",
        username            => $u->user,
        includewatchedusers => 1,
        includetrustedusers => 1
    );

    is(
        scalar @{ $res->{watchedusers} },
        scalar @watched,
        "Number of watched users did not change."
    );
    is(
        scalar @{ $res->{trustedusers} },
        scalar @trusted,
        "Number of trusted users did not change."
    );
    ...
    ( $res, $err ) = $do_request->(
        "editcircle",
        username => $u->user,
        add      => [ { username => $watchertruster->user, groupmask => 201 } ]
    );
    ( $res, $err ) = $do_request->(
        "getcircle",
        username            => $u->user,
    ...
    ( $res, $err ) = $do_request->(
        "editcircle",
        username => $u->user,
        add      => [ { username => $watchertruster->user, edge => 0b00 } ]
    );
    ...
    ( $res, $err ) = $do_request->(
        "editcircle",
        username => $u->user,
        add      => [ { username => $watchertruster->user, edge => 0b01 } ]
    );
    ...
    ( $res, $err ) = $do_request->(
        "editcircle",
        username => $u->user,
        add      => [ { username => $watchertruster->user, edge => 0b10 } ]
    );
    is( $u->trustmask($watchertruster), 0, "Not trusted." );
    ok( !$u->trusts($watchertruster), "Currently not trusted." );
    ...
    ( $res, $err ) = $do_request->(
        "editcircle",
        username => $u->user,
        add      => [ { username => $watchertruster->user, edge => 0b11 } ]
    );
    is( $u->trustmask($watchertruster), 1, "Just trust." );
    ok( $u->trusts($watchertruster),  "Currently trusted." );
    ok( $u->watches($watchertruster), "Currently watched." );
    ...
    ( $res, $err ) =
        $do_request->( "editcircle", username => $u->user, setcontentfilters => \%contentfilters );
    ...
    is(
        scalar @{ $res->{addedcontentfilters} },
        scalar keys %contentfilters,
        "Got back the newly-added content filters."
    );
    ...
    is(
        scalar @{ $res->{contentfilters} },
        scalar keys %contentfilters,
        "Number of content filters match."
    );
    ...
    ( $res, $err ) = $do_request->(
        "editcircle",
        username          => $u->user,
        setcontentfilters => { 2 => $edited, 3 => $new }
    );
    ...
    ( $res, $err ) =
        $do_request->( "editcircle", username => $u->user, deletecontentfilters => [ 1, 3 ] );
...
note("post to community by various journaltypes");
{
    ...
    DW::RequestCache->remove( 'rel', $comm->userid . "-" . $admin->userid . "-A" );    # to be safe
    ...
    ok(
        LJ::Tags::can_control_tags( $comm, $identity_u ),
        "Identity user can control tags on communities."
    );
    ok(
        LJ::Tags::can_add_tags( $comm, $identity_u ),
        "Identity user can control tags on communities."
    );
...
note("editing an entry with existing tags, when only admins can edit tags");
{
    ...
    ( $res, $err ) = $do_request->(
        ...
        event =>
            "new test post to a community containing tags when you're not allowed to have them",
    ...
    is(
        $res->{message},
        "You are not allowed to tag entries in this journal.",
...
note("checkforupdates");
{
    ...
    $LJ::CAP{$_}->{checkfriends} = 0 foreach ( 0 .. 15 );
    ...
    ( $res, $err ) = $do_request->( "checkforupdates", username => $u->user, );
...
# (not using $LJ::T_HAS_ALL_CAPS = 1, because that makes everyone readonly)
    $LJ::CAP{$_}->{checkfriends}          = 1 foreach ( 0 .. 15 );
    $LJ::CAP{$_}->{checkfriends_interval} = 7 foreach ( 0 .. 15 );
    ...
    ok( $earlierupdate < $laterupdate,
        "Timestamps need to be unequal for when we're testing stuff." );
...
{
    my $u = temp_user();
    ( $res, $err ) = $do_request->( "getfriendspage", username => $u->user );
...
note("getreadpage");
{
    ...
    # show everything
    ( $res, $err ) = $do_request->( "getreadpage", username => $u1->user, );
    @entries = @{ $res->{entries} };
