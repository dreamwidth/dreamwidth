...
sub delete_entry {
    ...
    if ($xpost_protocol) {
        ...
        if ($result->{success}) {
            ...
            $returnvalue{message} = LJ::Lang::ml("xpost.delete.success", { username => $self->username, server => $self->servername });
            $returnvalue{reference} = $result->{reference};
        } else {
            $returnvalue{success} = 0;
            $returnvalue{error} = LJ::Lang::ml("xpost.delete.error", { username => $self->username, server => $self->servername, error => $result->{error} });
        }
    } else {
        $returnvalue{success} = 0;
...
sub protocol {
    my $self = shift;
    my $servicetype = $self->externalsite ? $self->externalsite->{servicetype} : $self->{servicetype};
...
sub set_xpostbydefault {
    ...
    unless ($newvalue eq $self->xpostbydefault) {
        $u->do("UPDATE externalaccount SET xpostbydefault=? WHERE userid=? AND acctid=?", undef, $newvalue, $u->{userid}, $self->acctid);
        LJ::throw($u->errstr) if $u->err;
...
sub set_password {
    ...
    unless ($newvalue eq $self->password) {
        $u->do("UPDATE externalaccount SET password=? WHERE userid=? AND acctid=?", undef, $newvalue, $u->{userid}, $self->acctid);
        LJ::throw($u->errstr) if $u->err;
