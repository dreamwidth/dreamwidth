...
sub new_from_cart {
    ...
    my ( $gcoid, $cn, $em ) =
        $dbh->selectrow_array( 'SELECT gcoid, contactname, email FROM gco_map WHERE cartid = ?', undef, $cart->id );
...
sub new_from_gcoid {
    ...
    my ( $cartid, $cn, $em ) =
        $dbh->selectrow_array( 'SELECT cartid, contactname, email FROM gco_map WHERE gcoid = ?', undef, $gcoid );
    ...
    if ( $form->{_type} eq 'new-order-notification' ) {
        ...
        $dbh->do(
            q{INSERT INTO gco_map (gcoid, cartid, email, contactname)
              VALUES (?, ?, ?, ?)},
            undef, $gon, $cart->id, $form->{'buyer-billing-address.email'}, $form->{'buyer-billing-address.contact-name'}
        );
...
sub gco {
    return Google::Checkout::General::GCO->new( config_path => "$LJ::HOME/etc/google-checkout.conf" );
}
