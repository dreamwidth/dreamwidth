...
sub _wt_userids {
    ...
    if ( $mode eq 'watch' ) {
        if ( $reverse ) {
            $sql = "SELECT from_userid FROM wt_edges WHERE to_userid=? AND groupmask & 1<<61 LIMIT $limit";
            $memkey = [$u->id, "watched_by:" . $u->id];
        } else {
            $sql = "SELECT to_userid FROM wt_edges WHERE from_userid=? AND groupmask & 1<<61 LIMIT $limit";
            $memkey = [$u->id, "watched:" . $u->id];
        }

    } elsif ($mode eq 'trust') {
        if ( $reverse ) {
            $sql = "SELECT from_userid FROM wt_edges WHERE to_userid=? AND groupmask & 1 LIMIT $limit";
            $memkey = [$u->id, "trusted_by:" . $u->id];
        } else {
            $sql = "SELECT to_userid FROM wt_edges WHERE from_userid=? AND groupmask & 1 LIMIT $limit";
