...
sub save {
    ...
    $u->do( 'INSERT INTO vgift_trans (transid, rcptid, vgiftid, buyerid,' .
...
sub load {
    ...
    unless ( defined $data ) {
        $data = $u->selectrow_hashref( 'SELECT transid, rcptid, vgiftid,' .
        ...
        if ( my $item = $class->_search_cart( $data ) ) {
            ...
            $data->{from} = $item->from_html ne LJ::Lang::ml( 'error.nojournal' )
                          ? $item->from_html : undef;
            $data->{from_text} = $item->from_text ne LJ::Lang::ml( 'error.nojournal' )
