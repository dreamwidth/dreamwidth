...
sub time {
    ...
    my $raw_data = $self->{_raw_data};
    my $datestr =
        sprintf( "%04d-%02d-%02d", $raw_data->{year}, $raw_data->{mon}, $raw_data->{day} );
...
sub approve {
    ...
    if ($res) {
        ...
        my $entry =
            LJ::Entry->new( $self->journal, jitemid => $res->{itemid}, anum => $res->{anum} );
        return ( 1, $entry->url );
    }
...
sub notify_poster {
    ...
    my @metadata;
    push @metadata,
        LJ::Lang::ml( "$ml_scope.email.submission.music", { music => $props->{current_music} } )
        if $props->{current_music};
    push @metadata,
        LJ::Lang::ml( "$ml_scope.email.submission.mood", { mood => $props->{current_mood} } )
        if $props->{current_mood};
    push @metadata,
        LJ::Lang::ml( "$ml_scope.email.submission.icon", { icon => $props->{picture_keyword} } )
        if $props->{picture_keyword};

    my $subject =
        LJ::Lang::ml( "$ml_scope.email.submission.subject", { subject => $raw_data->{subject} } );
...
# the trailing spaces are unfortunately necessary to introduce line breaks...
    ...
    if ( $status eq 'approved' ) {
        ...
        $email_body .= "\n\n"
            . LJ::Lang::ml(
            "$ml_scope.email.body.approved.message",
            {
        ...
        $email_body =
            $opts{message}
            ? LJ::Lang::ml(
            "$ml_scope.email.body.rejected_with_message",
            {
                comm    => "@" . $journal->user,
                message => $opts{message}
            }
