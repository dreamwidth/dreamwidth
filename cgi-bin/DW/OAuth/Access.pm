...
sub from_token {
    ...
    my $cached = DW::Cache->request->get( 'oauth_access', $token );
...
sub from_consumer {
    ...
    my $cached = DW::Cache->request->get( 'oauth_access', "$uid:$cid" );
    return $cached if $cached;
...
sub _delete_cache {
    ...
    DW::Cache->request->remove( 'oauth_access', $c->token );
    DW::Cache->request->remove( 'oauth_access', $c->userid . ":" . $c->consumer_id );
}
...
"UPDATE oauth_access_token SET token = NULL, secret = NULL WHERE consumer_id = ? AND userid = ?",
    ...
    DW::Cache->request->remove( 'oauth_access', $old_token );
}
