...
sub from_consumer {
    ...
    return $LJ::REQUEST_CACHE{oauth_access}{"$uid:$cid"} if $LJ::REQUEST_CACHE{oauth_access}{"$uid:$cid"};
...
sub tokens_for_user {
    ...
    if ( $data ) {
        ...
        my $sth = $dbr->prepare( "SELECT consumer_id FROM oauth_access_token WHERE userid = ?" ) or die $dbr->errstr;
...
sub tokens_for_consumer {
    ...
    my $dbr = LJ::get_db_reader() or die "Failed to get database";
    my $sth = $dbr->prepare( "SELECT userid FROM oauth_access_token WHERE consumer_id = ?" ) or die $dbr->errstr;
...
sub _delete_cache {
    ...
    LJ::MemCache::delete( [ $c->userid, join(":","oauth_access", $c->userid, $c->consumer_id ) ] );
...
sub _load_raw {
    ...
    my $dbr = LJ::get_db_reader() or die "Failed to get database";
    my $sth = $dbr->prepare( "SELECT consumer_id, userid, token, secret, createtime FROM oauth_access_token WHERE $data" ) or die $dbr->errstr;
...
sub new_from_row {
    ...
    if ( $c->token ) {
        LJ::MemCache::set( [ $c->token, "oauth_access_token:" . $c->token ] , [ $c->userid, $c->consumer_id ], $expire );
        $LJ::REQUEST_CACHE{oauth_access}{ $c->token } = $c;
    }

    my $ar = LJ::MemCache::hash_to_array( "oauth_access", $c );
    LJ::MemCache::set( [ $c->userid, join(":","oauth_access", $c->userid, $c->consumer_id ) ] , $ar, $expire );
...
sub load_all_lastaccess {
    ...
    foreach my $userid ( keys %userids ) {
        ...
        my $sth = $dbr->prepare( "SELECT consumer_id,lastaccess FROM oauth_access_token WHERE consumer_id IN ($qmarks) AND userid = ?" ) or die $dbr->errstr;
...
sub update_accessed {
    ...
    $dbh->do( "UPDATE oauth_access_token SET lastaccess = ? WHERE consumer_id = ? AND userid = ?" , undef, time, $self->consumer_id, $self->userid ) or die
        $dbh->errstr;
...
sub invalidate_token {
    ...
    my $dbh = LJ::get_db_writer();
    $dbh->do( "UPDATE oauth_access_token SET token = NULL, secret = NULL WHERE consumer_id = ? AND userid = ?" , undef,
        $c->consumer_id, $c->userid ) or die $dbh->errstr;
...
sub reissue_token {
    ...
    my $dbh = LJ::get_db_writer();
    $dbh->do( "UPDATE oauth_access_token SET createtime = ?, token = ?, secret = ? WHERE consumer_id = ? AND userid = ?" , undef,
        time(), $token, $secret, $c->consumer_id, $c->userid ) or die $dbh->errstr;
...
sub delete {
    ...
    $dbh->do("DELETE FROM oauth_access_token WHERE userid = ? AND consumer_id = ?",undef,$c->userid,$c->consumer_id)
        or return 0;
