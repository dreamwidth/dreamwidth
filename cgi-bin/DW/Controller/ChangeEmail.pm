...
sub changeemail_handler {
    ...
    if ( $r->did_post && ( $post->{email} || $post->{password} ) ) {
        ...
        if ( $LJ::USER_EMAIL and $post->{email} =~ /\@\Q$LJ::USER_DOMAIN\E$/i ) {
            push @errors, LJ::Lang::ml( "/changeemail.tt.error.lj_domain2", { 'user' => $remote->{'user'}, 'domain' => $LJ::USER_DOMAIN, 'aopts' => "href='$LJ::SITEROOT/manage/profile/'" } );
        }
        ...
        unless ( scalar @errors ) {
            ...
            LJ::send_mail( {
                ...
                'body'     => LJ::Lang::ml( '/changeemail.tt.newemail_old.body2',
            ...
            if ( $is_identity_no_email ) {
                ...
                'body'     => LJ::Lang::ml( '/changeemail.tt.newemail.body.openid',
                                  { username => $u->display_username,
                                    sitename => $LJ::SITENAME,
                                    sitelink => $LJ::SITEROOT,
                ...
                LJ::send_mail( {
                    ...
                    'body'     => LJ::Lang::ml( '/changeemail.tt.newemail.body3',
                                      { username => $u->display_username,
                                        email    => $u->email_raw,
                                        sitename => $LJ::SITENAME,
                                        sitelink => $LJ::SITEROOT,
