...
sub search_handler {
    ...
    $rv->{tagprint} = sub { join( ', ', map { "<strong>" . $_[0]->{$_} . "</strong>" }
                                        keys %{ $_[0] } ) };
    $rv->{sec_icon} = sub { return {
            public  => '',
...
my $mock_results = { total => 2, time => '0.00', matches => [
                       { journalid => 1, poster_id => 0, security => 'public',
                         jtalkid => 0, url => 'blank', subject => 'testing',
                         excerpt => 'text goes here...', eventtime => LJ::mysql_time,
                         tags => { 1 => 'foo', 2 => 'bar' } },
                       { journalid => 1, poster_id => 0, security => 'access',
                         jtalkid => 0, url => 'blank', subject => 'testing',
                         excerpt => 'text goes here...', eventtime => LJ::mysql_time,
                         tags => {} },
                     ] };
