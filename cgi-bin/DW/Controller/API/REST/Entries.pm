...
$route->path (
	...
	$route->param({name => 'entry_id', type => 'integer', desc => 'The id of the entry you want information for.', in => 'path'})
);

# define our parameters and options for GET requests
my $get = $route->get('Returns recent entries for a specified journal, or a single entry for a specified entry id and journal', \&rest_get);
