...
sub feedping_handler {
    ...
    return $out->(
"This is a REST-like interface for pinging $LJ::SITENAMESHORT feed crawler to re-fetch a syndication URL.  Do a POST to this URL with a 'feed' parameter equal to the URL.  Possible HTTP responses are 400 (bad request), 404 (we're not indexing that feed), or 204 (we'll get to it soon).  (also permitted are multiple feed parameters, if you're not sure we're indexing your Atom vs RSS, etc.  At most 3 are currently accepted.)"
    ) unless $r->did_post;
...
sub whereami_handler {
    ...
    my $vars = {
        ...
        cluster_name => $LJ::CLUSTER_NAME{ $rv->{u}->clusterid }
            || LJ::Lang::ml('/misc/whereami.tt.cluster.unknown'),
    };
...
sub community_guidelines {
    ...
    return error_ml( 'error.guidelines.none',
        { user => $u->ljuser_display, aopts => "href='" . $u->profile_url . "'" } )
