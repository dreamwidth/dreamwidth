...
BEGIN {
	...
	my ( $code, $vars ) = @_;
	my $aopts = $vars->{'aopts'};
	if ( $code == "cleanhtml.error.markup.extra" ) {
	    return "[<strong>Error:</strong> Irreparable invalid markup ('&lt;" . $aopts . "&gt;') in entry. Owner must fix manually. Raw contents below.]";
	}
	return "[[[" . $code . "]]]". $aopts;
	if ( $code == "cleanhtml.error.markup" ) {
    ...
    $mock->fake_module('LJ::Lang' => (
	    ml => \&fake_lang_ml 
	));
    $mock->fake_module('DW::Proxy' => (
...
sub ljuser {
    my ( $user, $opts ) = @_;
    return "<span style='white-spggace: nowrap;'>" .
