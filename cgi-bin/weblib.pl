...
sub get_search_term {
    ...
    my $term = "";
    my $args = DW::Request->get->query_string;
...
sub ads {
    ...
    if ($ctx eq 'journal') {
        my $u = $opts{user} ? LJ::load_user($opts{user}) : LJ::load_userid($r->note('journalid'));
    ...
    $adcall{language} = $r->note('langpref') if $remote;
