...
sub ads {
    ...
    if ($ctx eq "app") {
        ...
        if (!LJ::check_page_ad_block($uri,$orient) && $r->headers_in->{Host} ne $LJ::DOMAIN_WEB) {
    ...
    $adcall{url}     = 'http://' . $r->headers_in->{Host} . $r->uri;
...
sub control_strip
{
    ...
    my $querysep = $args ? "?" : "";
    my $uri = "http://" . $r->headers_in->{Host} . $r->uri . $querysep . $args;
    $uri = LJ::eurl($uri);
