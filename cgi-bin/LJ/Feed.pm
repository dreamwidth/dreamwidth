...
sub create_view_foaf {
    ...
    if ( $u->{'country'} ) {
        ...
        $ret .=
"    <ya:country dc:title=\"$ecountry\" rdf:resource=\"$LJ::SITEROOT/directorysearch?opt_sort=ut&amp;s_loc=1&amp;loc_cn=$ecountry\"/>\n"
            if $u->can_show_location($remote);
        if ( $u->{'city'} ) {
            my $estate = '';                         # FIXME: add state.  Yandex didn't need it.
            my $ecity  = LJ::eurl( $u->{'city'} );
            $ret .=
"    <ya:city dc:title=\"$ecity\" rdf:resource=\"$LJ::SITEROOT/directorysearch?opt_sort=ut&amp;s_loc=1&amp;loc_cn=$ecountry&amp;loc_st=$estate&amp;loc_ci=$ecity\"/>\n"
                if $u->can_show_location($remote);
        }
    }
    ...
    $ret .=
"        <dc:description>Full $LJ::SITENAME profile, including information such as interests and bio.</dc:description>\n";
    $ret .= "      </foaf:Document>\n";
    $ret .= "    </foaf:page>\n";
    ...
    if ( $u->{allow_contactshow} eq 'Y' ) {
        foreach my $type ( keys %types ) {
            ...
            $ret .=
                "    <foaf:$types{$type}>" . LJ::exml( $u->{$type} ) . "</foaf:$types{$type}>\n";
        }
    }
    ...
    # blog activity
    {
        ...
        $ret .=
            ...
            . $u->journal_base
            . "/data/rss\" dc:type=\"application/rss+xml\" />\n";
    ...
    if ( $u->{icbm} ) {
        my @loc = split( ",", $u->{icbm} );
        $ret .=
              "    <foaf:based_near><geo:Point geo:lat='"
            . $loc[0] . "'"
            . " geo:long='"
            . $loc[1]
            . "' /></foaf:based_near>\n";
    ...
    foreach my $int (@$intu) {
        ...
        $ret .=
              "    <foaf:interest dc:title=\""
            . LJ::exml( $int->[1] ) . "\" "
            . "rdf:resource=\"$LJ::SITEROOT/interests?int="
            . LJ::eurl( $int->[1] )
            . "\" />\n";
    }
