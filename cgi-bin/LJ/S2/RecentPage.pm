...
sub RecentPage {
    ...
    # Link to the friends page as a "group", for use with OpenID "Group Membership Protocol"
    {
        ...
        my $friendsurl =
            $u->journal_base . "/read";    # We want the canonical form here, not the vhost form
        $p->{head_content} .=
              '<link rel="'
            . $rel
            . '" title="'
            . LJ::ehtml($friendstitle)
            . '" href="'
            . LJ::ehtml($friendsurl)
            . "\" />\n";
    }
    ...
    my $datalink = sub {
        ...
        return Link(
            $p->{'base_url'}
                ...
                . (
                $opts->{tags}
                ? "?tag=" . join( ",", map( { LJ::eurl($_) } @{ $opts->{tags} } ) )
                : ""
                ),
            $caption,
            Image_std($what)
  ...
  expandAll = '$expandAll';
    ...
    my $show_sticky_entries =
        $skip == 0 && !$opts->{securityfilter} && !$opts->{tagids} && !$posteru_filter;
...
# don't show posts from suspended users or suspended posts unless the user doing the viewing says to (and is allowed)
    ...
    if ($skip) {
        ...
        $nav->{'forward_url'} =
            LJ::S2::make_link( "$p->{'base_url'}/", { skip => $newskip || "", %linkattrs } );
