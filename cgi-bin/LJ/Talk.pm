...
sub load_comments {
    ...
    if ( ref( $opts->{userref} ) eq "HASH" ) {
        ...
        if ( ref( $opts->{userpicref} ) eq "HASH" ) {
            ...
            LJ::Userpic::load_userpics( $opts->{userpicref}, \@load_pic );
        }
    }
...
# Lives in LJ::Userpic (the userpic2 owner); delegated here for external callers.
sub load_userpics { return LJ::Userpic::load_userpics(@_); }
