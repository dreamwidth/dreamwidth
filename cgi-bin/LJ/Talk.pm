...
sub authenticate_user_and_mutate_form {
    ...
    my $incoherent = sub {
        return $mlerr->("$SC.error.confused_identity");
    };
    ...
    if ( $form->{usertype} eq 'anonymous' ) {
        ...
        if ( $remote && ($remote->user eq $form->{cookieuser}) ) {
            ...
            return $mlerr->("$SC.error.lostcookie");
        }
        ...
        if ( ! $form->{userpost} ) {
            ...
            return $mlerr->( "$SC.error.nousername$noanon$iscomm", { sitename => $LJ::SITENAMESHORT } );
        }
        ...
        if ( $up->is_community || $up->is_syndicated ) {
            return $mlerr->("$SC.error.postshared");
        }
...
"Sorry, form expired.  Press back, copy text, reload form, paste into new form, and re-submit."
    ...
    $init->{didlogin} = 0;
    my $commenter_or_redirect = authenticate_user_and_mutate_form($form, $remote, $journalu, $errret, \$init->{didlogin});
    if (defined $commenter_or_redirect && ! $commenter_or_redirect->isa('LJ::User')) {
        # openid thing
        return { check_url => $commenter_or_redirect };
    }

    my $comment = prepare_and_validate_comment($form, $commenter_or_redirect, $item, $need_captcha, $errret);
