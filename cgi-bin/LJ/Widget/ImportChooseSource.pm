...
sub render_body {
    ...
    $ret .= "<p>"
        . $class->ml( 'widget.importchoosesource.intro', { sitename => $LJ::SITENAMESHORT } )
        . "</p>";
    ...
    $ret .=
        "<label for='username'>" . $class->ml('widget.importchoosesource.username') . "</label>";
    $ret .= $class->html_text( name => 'username', maxlength => 255 );
    $ret .= "</div>";
    $ret .= "<div class='i-password'>";
    $ret .=
        "<label for='password'>" . $class->ml('widget.importchoosesource.password') . "</label>";
    ...
    if ( $u->is_community ) {
        ...
        $ret .=
              "<label for='usejournal'>"
            . $class->ml('widget.importchoosesource.usejournal')
            . "</label>";
