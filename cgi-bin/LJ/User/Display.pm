...
sub bio {
    ...
    my $db =
        @LJ::MEMCACHE_SERVERS || $force
        ? LJ::get_cluster_def_reader($u)
        : LJ::get_cluster_reader($u);
    return unless $db;
    $bio = $db->selectrow_array( "SELECT bio FROM userbio WHERE userid=?", undef, $u->userid );
...
sub set_bio {
    ...
    if ( $has_bio eq "N" ) {
        ...
        $u->do( "REPLACE INTO userbio (userid, bio) VALUES (?, ?)", undef, $u->id, $text );
...
sub get_timezone {
    ...
    if ( my $tz = $u->prop('timezone') ) {
        ...
        my $dt = eval { DateTime->from_epoch( epoch => time(), time_zone => $tz, ); };
    ...
    if (
        my $last_row = $dbcr->selectrow_hashref(
            qq{
...
sub ljuser {
    ...
    my $make_tag = sub {
        ...
        if (   $opts->{'link_color'}
            && $opts->{'link_color'} =~ /^#([a-fA-F0-9]{3}|[a-fA-F0-9]{6})$/ )
        {
    ...
    if ( $type eq 'C' ) {
        ...
        return $make_tag->( "silk/${head_size}/feed.png", $url, $head_size, '', $type_readable )
            if $head_size;
