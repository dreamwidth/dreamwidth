...
sub create {
    ...
    $dbh->do( 'INSERT INTO useridmap (userid, user) VALUES (?, ?)', undef, $userid, $username );
    return $err->( 'Database error: ', $dbh->errstr ) if $dbh->err;

    $dbh->do( 'INSERT INTO userusage (userid, timecreate) VALUES (?, NOW())', undef, $userid );
    return $err->( 'Database error: ', $dbh->errstr ) if $dbh->err;
...
sub create_community {
    ...
    $u->set_prop( "adult_content",     $opts{journal_adult_settings} )
...
sub create_personal {
    ...
    if ( $opts{code} ) {
        ...
        my $itemidref;
        my $promo_code =
            $LJ::USE_ACCT_CODES ? DW::InviteCodes::Promo->load( code => $code ) : undef;
        if ($promo_code) {
            ...
            if ( $item && $item->isa('DW::Shop::Item::Account') ) {
                ...
                if ( DW::Pay::add_paid_time( $u, $item->class, $item->months ) ) {
                    ...
                    LJ::statushistory_add(
                        ...
                        'paid_from_invite',
                        sprintf(
                            "Failed to create new '%s' account from order #%d: %s",
...
sub delete_and_purge_completely {
    ...
    $dbh->do( "DELETE FROM wt_edges WHERE from_userid = ? OR to_userid = ?", undef, $u->id,
        $u->id );
...
sub load_random_user {
    ...
    my $uids = $dbcr->selectcol_arrayref(
        qq{
...
sub timecreate {
    ...
    my $when =
        $dbr->selectrow_array( "SELECT timecreate FROM userusage WHERE userid=?", undef, $u->id );
...
sub identity {
    ...
    my $dbh = LJ::get_db_writer();
    $ident = $dbh->selectrow_arrayref(
        "SELECT idtype, identity FROM identitymap " . "WHERE userid=? LIMIT 1",
        undef, $u->userid );
    if ($ident) {
...
sub load_identity_user {
    ...
    my $dbh = LJ::get_db_writer();
    return undef
        unless $dbh->do( "INSERT INTO identitymap (idtype, identity, userid) VALUES (?,?,?)",
...
sub rename_identity {
    ...
    my $tempid = sub {
        ...
        for ( 1 .. 10 ) {
            ...
            unless (
                $dbh->selectrow_array(
                    "SELECT COUNT(*) FROM identitymap WHERE identity=? AND idtype=? LIMIT 1",
                    undef, $exid, $idtype
                )
                )
            {
...
sub canonical_username {
    my $input = lc( $_[0] // '' );
    my $user  = "";
    if ( $input =~ /^\s*([a-z0-9_\-]{1,$LJ::USERNAME_MAXLENGTH})\s*$/ ) {
        $user = $1;
        $user =~ s/-/_/g;
    }
    return $user;
...
sub get_username {
    ...
    unless ( defined $user ) {
        my $dbh = LJ::get_db_writer();
        $user =
            $dbh->selectrow_array( "SELECT user FROM useridmap WHERE userid=?", undef, $userid );
    }
...
sub load_user {
    ...
    if ( $u = DW::RequestCache->get( 'user_name', $user ) ) {
        $u->selfassert;
        return $u;
    }
...
sub load_userid {
    ...
    # check process cache
    $u = DW::RequestCache->get( 'user_id', $userid );
    if ($u) {
        $u->selfassert;
        return $u;
    }
...
sub load_userids_multiple {
    ...
    while (@$map) {
        ...
        if ( my $cached = DW::RequestCache->get( 'user_id', $id ) ) {
            push @have, $cached;
        }
    }
    ...
    if ( %need && !$memcache_only ) {
        my $db = @LJ::MEMCACHE_SERVERS
            || $LJ::_PRAGMA_FORCE_MASTER ? LJ::get_db_writer() : LJ::get_db_reader();
