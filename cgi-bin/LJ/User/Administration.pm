...
sub dudata_set {
    ...
    if ($bytes) {
        $u->do(
            "REPLACE INTO dudata (userid, area, areaid, bytes) " . "VALUES (?, ?, $areaid, $bytes)",
            undef, $u->userid, $area
        );
    }
    else {
        $u->do( "DELETE FROM dudata WHERE userid=? AND " . "area=? AND areaid=$areaid",
            undef, $u->userid, $area );
    }
    return 1;
}
...
sub infohistory_add {
    ...
    $dbh->do(
"INSERT INTO infohistory (userid, what, timechange, oldvalue, other) VALUES (?, ?, ?, ?, ?)",
...
sub log_event {
    ...
    my $extra =
        %$info
        ? join( '&', map { LJ::eurl($_) . '=' . LJ::eurl( $info->{$_} ) } sort keys %$info )
        : undef;
...
sub rate_check {
    ...
    my $sth =
        $udbr->prepare( "SELECT evttime, quantity FROM ratelog WHERE "
    ...
    if ( defined $text ) {
        ...
        my $data = $dbr->selectall_arrayref(
            "SELECT banid, remoteid, notetext FROM bannotes "
                . "WHERE journalid=? AND banid IN ($qs)",
            undef, $u->id, @banned
        );
