...
sub send_comm_invite {
    ...
    $cu->do(
        'DELETE FROM invitesent WHERE commid = ? AND '
            . 'recvtime < UNIX_TIMESTAMP(DATE_SUB(NOW(), INTERVAL 30 DAY))',
        undef, $cu->{userid}
    );
    ...
    my $argstr =
        $dbcr->selectrow_array( 'SELECT args FROM inviterecv WHERE userid = ? AND commid = ?',
    ...
    unless ($argstr) {
        ...
        my $count = $cdbcr->selectrow_array(
            "SELECT COUNT(*) FROM invitesent WHERE commid = ? "
    ...
    if ($argstr) {
        ...
        $cu->do(
"UPDATE invitesent SET args = ?, status = 'outstanding' WHERE commid = ? AND userid = ?",
...
sub accept_comm_invite {
    ...
    my $dbcr = LJ::get_cluster_def_reader($u);
    return LJ::error('db') unless $dbcr;
    my $argstr = $dbcr->selectrow_array(
        'SELECT args FROM inviterecv WHERE userid = ? AND commid = ? '
    ...
    my $joined = 0;
    $joined = $u->join_community( $cu, 1, 0, moderated_add => 1 )
        or return undef
        if $args->{member};
...
sub reject_comm_invite {
    ...
    my $dbcr = LJ::get_cluster_def_reader($u);
    return LJ::error('db') unless $dbcr;
    my $test = $dbcr->selectrow_array(
        'SELECT userid FROM inviterecv WHERE userid = ? AND commid = ? '
            . 'AND recvtime > UNIX_TIMESTAMP(DATE_SUB(NOW(), INTERVAL 30 DAY))',
        undef, $u->{userid}, $cu->{userid}
    );
...
sub get_sent_invites {
    ...
    my $data = $dbcr->selectall_arrayref(
        'SELECT userid, maintid, recvtime, status, args FROM invitesent '
...
sub get_pending_invites {
    ...
    my $dbcr = LJ::get_cluster_def_reader($u);
    return LJ::error('db') unless $dbcr;
    my $pending = $dbcr->selectall_arrayref(
        'SELECT commid, maintid, recvtime, args FROM inviterecv WHERE userid = ? '
            . 'AND recvtime > UNIX_TIMESTAMP(DATE_SUB(NOW(), INTERVAL 30 DAY))',
        undef, $u->{userid}
    );
...
sub revoke_invites {
    ...
    return LJ::error('db') unless $cu->writer;
    $cu->do( "DELETE FROM invitesent WHERE commid = ? AND " . "userid IN ($in)",
        undef, $cu->{userid} );
...
sub get_pending_members {
    ...
    # database request
    my $dbr  = LJ::get_db_reader() or return;
    my $args = $dbr->selectcol_arrayref(
        'SELECT arg1 FROM authactions WHERE userid = ? '
...
sub comm_join_request {
    ...
    my $oldaa = $dbh->selectrow_hashref(
    ...
    $dbh->do(
        "UPDATE authactions SET used='Y' WHERE userid=? AND aaid<>? AND arg1=? "
...
sub set_comm_settings {
    ...
    my $dbh = LJ::get_db_writer();
    $dbh->do(
"INSERT INTO community (userid, membership, postlevel) VALUES (?,?,?) ON DUPLICATE KEY UPDATE $updates",
        undef,
        $cid,
...
sub get_member {
    ...
    # get all community edges
    my $results = $dbr->selectall_arrayref(
        "SELECT r.targetid, r.type, u.user FROM reluser r, useridmap u "
            . "WHERE r.targetid = u.userid AND r.userid=? AND r.targetid=?",
        undef, $cu->userid, $u->userid
    );
...
sub _notify_administrator {
    ...
    if ( $action eq "maintainer_add" ) {
        $email_body = LJ::Lang::ml(
            "$ml_scope.email.admin.add.body",
            {
                comm          => $cu->name_raw,
        ...
        $email_subject =
            LJ::Lang::ml( "$ml_scope.email.admin.add.subject", { comm => $cu->name_raw } );
    }
    else {
        $email_body = LJ::Lang::ml(
            "$ml_scope.email.admin.delete.body",
            {
                admin => $remote->user,
        ...
        $email_subject =
            LJ::Lang::ml( "$ml_scope.email.admin.delete.subject", { comm => $cu->name_raw } );
    }
