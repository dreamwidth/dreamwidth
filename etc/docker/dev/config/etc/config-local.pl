...

{
    ...
    # setup textcaptcha
    %TEXTCAPTCHA = ( api_key => $DW::PRIVATE::TEXTCAPTCHA{api_key}, );
 ...
 #        api_url   => 'https://api-3t.sandbox.paypal.com/nvp',
    ...
    #    icons => [],      # if present, sell icons
    #    vgifts => [],     # if present, sell virtual gifts
    #    rename => [ 15, undef, undef, 150 ],
    #);

    # minimum amount for check/money order (0 for no minimum)
    # $SHOP_CMO_MINIMUM = 20;
    ...
    %KNOWN_HTTPS_SITES = map { $_ => 1 } qw/ imgur.com yandex.ru xkcd.com abload.de tumblr.com
    ...
    # Set to true if this is a canary server. This causes a very visible banner to appear
    # on every page rendered.
    $IS_CANARY = 0;
}
