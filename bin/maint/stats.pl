    ...
    LJ::Stats::register_stat(
        {
            ...
            'handler'  => sub {
                ...
                my $sth = $db->prepare(
                          "SELECT k.keyword, i.intcount FROM interests AS i, sitekeywords AS k "
    ...
    LJ::Stats::register_stat(
        {
            ...
            'handler'  => sub {
                ...
                foreach my $block ( 1 .. $blocks ) {
                    ...
                    my $sth = $db->prepare(
                              "SELECT c.client, COUNT(*) AS 'count' FROM clients c, clientusage cu "
    ...
    LJ::Stats::register_stat(
        {
            ...
            'handler'  => sub {
                ...
                my %ret
                    ;  # return hash, (statname => { arg => val } since 'statname' is arrayref above
                ...
                foreach my $block ( 1 .. $blocks ) {
                    ...
                    my $sth =
                        $db->prepare( "SELECT DATE_FORMAT(timecreate, '%Y-%m-%d') AS 'datereg', "
                    ...
                    while ( my $rec = $sth->fetchrow_hashref ) {
                        ...
                        if ( my $time = $rec->{'timeupdate'} ) {
                            ...
                            $ret{'userinfo'}->{'updated_last30'}++
                                if $time > $now - 60 * 60 * 24 * 30;
                            $ret{'userinfo'}->{'updated_last7'}++
                                if $time > $now - 60 * 60 * 24 * 7;
                            $ret{'userinfo'}->{'updated_last1'}++
                                if $time > $now - 60 * 60 * 24 * 1;
                        }
                    }
    ...
    LJ::Stats::register_stat(
        {
            ...
            'handler'  => sub {
                ...
                foreach my $block ( 1 .. $blocks ) {
                    ...
                    $db = $db_getter->();    # revalidate connection
                    my $sth =
                        $db->prepare( "SELECT u.value, COUNT(*) AS 'count' FROM userproplite2 u "
    ...
    LJ::Stats::register_stat(
        {
            ...
            'handler'  => sub {
                ...
                foreach my $block ( 1 .. $blocks ) {
                    ...
                    $db = $db_getter->();    # revalidate connection
                    my $sth =
                        $db->prepare( "SELECT ua.value, COUNT(*) AS 'count' "
    ...
    LJ::Stats::register_stat(
        {
            ...
            'handler'  => sub {
                ...
                foreach my $block ( 1 .. $blocks ) {
                    ...
                    $db = $db_getter->();    # revalidate connection
                    my $sth =
                        $db->prepare( "SELECT value, COUNT(*) AS 'count' FROM userproplite2 up "
    ...
    LJ::Stats::register_stat(
        {
            ...
            'handler'  => sub {
                ...
                my $sth =
                    $db->prepare( "SELECT FLOOR((UNIX_TIMESTAMP()-timeactive)/86400), COUNT(*) "
                        . "FROM clustertrack2 "
    ...
    LJ::Stats::register_stat(
        {
            ...
            'handler'  => sub {
                ...
                my $rows = $db->selectall_arrayref(
                    "SELECT statkey, statval FROM stats WHERE statcat = 'supportrank'");
                return {} unless $rows;
    ...
    LJ::Stats::register_stat(
        {
            ...
            'handler'  => sub {
                ...
                my $sth =
                    $db->prepare( "SELECT userid, SUM(points) AS 'points' "
                        . "FROM supportpoints "
